---
title: GitLab.com Database Outage
incident_id: real-public-incident
service: gitlab.com
severity: SEV1
date: 2017-01-31
source: https://about.gitlab.com/blog/postmortem-of-database-outage-of-january-31/
tags: [postgresql, database, backup, real-world, external]
note: This is a REAL, publicly published postmortem from GitLab, added as
  supplementary real-world incident data alongside this knowledge base's
  synthetic Shopify-platform documents -- not a Shopify-platform incident.
---

# Postmortem: GitLab.com Database Outage (January 31, 2017)
...
are sent by email. For GitLab.com we use DMARC.
...
- A snapshot created for our staging environment every 24 hours, almost 24
hours before the outage happened.
- A snapshot created manually by one of the engineers roughly 6 hours before
the outage.
When we generate a snapshot the following steps are taken:
- Generate a snapshot of production.
- Copy the snapshot to staging.
- Create a new disk using this snapshot.
- Remove all webhooks from the resulting database, to prevent them from being
...
- Copy the existing staging database to production, which would not contain any
webhooks.
- In parallel, copy the snapshot used to set up the database as this snapshot
might still contain the webhooks (we weren't entirely sure).
- Set up a production database using the snapshot from step 1.
- Set up a separate database using the snapshot from step 2.
- Restore webhooks using the database set up in the previous step.
- Increment all database sequences by 100,000 so one can't re-use IDs that
might have been used before the outage.
- Gradually re-enable GitLab.com.
...
- Copy over the LVM snapshot
- Copy over the PostgreSQL data directory
...
publicly visible Google document.
...
- Why could we not use the standard backup procedure? - The standard backup
...
- Why did the backup procedure fail silently? - Notifications were
...
- Why were the Emails rejected? - Emails were rejected by the receiving
mail server due to the Emails not being signed using DMARC.
- Why were Azure disk snapshots not enabled? - We assumed our other backup
procedures were sufficient. Furthermore, restoring these snapshots can take
days.
- Why was the backup procedure not tested on a regular basis? - Because
...
- Overview of status of all issues listed in this blog post (#1684)
...
doing so would only protect themselves against accidentally running rm -rf /important-data, not against disk corruption or any of the many other ways you
can lose data.
An ideal environment is one in which you can make mistakes but easily and
...
"Disaster recovery for everything that is not the database".
...
- Close the MR and create a new one
- Push new changes to the merge request's source branch
- Rebase/amend, and force push to the merge request's source branch
